...
head(datRandom)
```
First thing is to impute the data.  We want the data above, because all of the categorical variables are dicotmized  
```{r}
attach(datRandom)
...
combineEst = function(x){
  y = data.frame(summary(x)$coefficients, summary(x)$standard.errors)
  #pars = y[3,]
}
...
randomTestAll = rbind(randomTest1, randomTest2, randomTest3, randomTest4, randomTest5)


p_valueRandomFun = function(x){ 
  z = summary(x)$coefficients/summary(x)$standard.errors
  p <- (1 - pnorm(abs(z), 0, 1))*2
  p
}

randomTest1 = p_valueRandomFun(randomTest1)
randomTest1

randomTest2 = p_valueRandomFun(randomTest2)
randomTest2

randomTest3 = p_valueRandomFun(randomTest3)
randomTest3

randomTest4 = p_valueRandomFun(randomTest4)
randomTest4

randomTest5 = p_valueRandomFun(randomTest5)
