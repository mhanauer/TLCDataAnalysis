...
The for loop through each data set should work and is working for the betas.  The results are actually the same for each data set, but it does run it through all five.  However, doesn't work with just ols.out.  Not sure why not.  Doesn't seem to work with other functions like describe or other functions that I have created.
```{r}
...
m =5
for(i in 1:m) {
  mean.sd.out[[i]] = a.out$imputations[[i]]
}
mean.sd.out
