...
Ok here need to run reshape five times on previous imputted data sets so they are the same.
```{r}
datAnalysisAll
head(datRandom)

datAnalysis1 = reshape(datAnalysis[[1]], varying = list(c("Pre_RAS_PersonalConfidenceHope", "Post_RAS_PersonalConfidenceHope"), c("Pre_RAS_GoalSuccessOrientation", "Post_RAS_GoalSuccessOrientation"), c("Pre_INQ_PB", "Post_INQ_PB")), idvar = "ï..RID", direction = "long", times = c(0,1))

datAnalysis2 = reshape(datAnalysis[[2]], varying = list(c("Pre_RAS_PersonalConfidenceHope", "Post_RAS_PersonalConfidenceHope"), c("Pre_RAS_GoalSuccessOrientation", "Post_RAS_GoalSuccessOrientation"), c("Pre_INQ_PB", "Post_INQ_PB")), idvar = "ï..RID", direction = "long", times = c(0,1))

datAnalysis3 = reshape(datAnalysis[[3]], varying = list(c("Pre_RAS_PersonalConfidenceHope", "Post_RAS_PersonalConfidenceHope"), c("Pre_RAS_GoalSuccessOrientation", "Post_RAS_GoalSuccessOrientation"), c("Pre_INQ_PB", "Post_INQ_PB")), idvar = "ï..RID", direction = "long", times = c(0,1))
...
#datAnalysis = data.frame(datAnalysis, Treat2, Treat3)
```

Now we need impute the new long data set that will be used for the multilevel modeling analysis.
```{r}
m = 5
a.out = amelia(x = datAnalysis, m=m, noms = c("Gen", "Hisp", "Race", "SexOrientation",  "RelStatus", "Education", "Employment", "NumericPackage"))
...
```{r}
meldAllOneTailed = function(x,y){
  coefsAll = mi.meld(q = x, se = y)
  coefs1 = t(data.frame(coefsAll$q.mi))
  ses1 = t(data.frame(coefsAll$se.mi))
  z_stat = coefs1/ses1
  options(scipen=999)
  p = round((1-pnorm(abs(z_stat), 0, 1)),3)
  return(p)
}
...
se_output = NULL
for(i in 1:m){
  output[[i]] = lme(Pre_RAS_PersonalConfidenceHope ~  factor(NumericPackage)*time, random = ~ 1 | ï..RID, data =    datAnalysis[[i]])
...
meldAllOneTailed(coef_output, se_output)
```
